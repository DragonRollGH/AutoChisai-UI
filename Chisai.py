...
class Chisai_TJ:
    ...
    def isActivated(self):
        Url = 'https://tjxsfw.chisai.tech/api/school_tjxsfw_student/tblStudentUsers/isActivated?studentPid={}'.format(
            self.POST['studentPid'])
        try:
            Requ = self.session.get(url=Url, headers=self.headers)
        except Exception as Error:
    ...
    def csQDef(self):
        Url = 'https://tjxsfw.chisai.tech/api/school_tjxsfw_student/csQDef/?studentPid={}'.format(
            self.POST['studentPid'])
        try:
            Requ = self.session.get(url=Url, headers=self.headers)
        except Exception as Error:
    ...
    def yqfkQVaccine1(self):
        Url = 'https://tjxsfw.chisai.tech/api/school_tjxsfw_student/yqfkQVaccine1/byId?studentPid={}'.format(
            self.POST['studentPid'])
        try:
            Requ = self.session.get(url=Url, headers=self.headers)
        except Exception as Error:
    ...
    def hasDoneToday(self):
        Url = 'https://tjxsfw.chisai.tech/api/school_tjxsfw_student/yqfkLogDailyreport/hasDoneToday?studentPid={}'.format(
            self.POST['studentPid'])
        try:
            Requ = self.session.get(url=Url, headers=self.headers)
        except Exception as Error:
    ...
    def SendEmail(self, title, content, type='plain'):
        ...
        try:
            ...
            Title = 'Fail to Send Email: {}'.format(
                self.POST['studentStudentno'])
            self.LOG(Title)
...
if len(sys.argv) == 1:
    for name, user in conf.items():
        if user['Chisai'] == 1:
            Chisais.append(Chisai_TJ(user))
elif len(sys.argv) == 2:
    for name, user in conf.items():
        if name == sys.argv[1]:
            Chisais.append(Chisai_TJ(user))
